...
class Agent:
    ...
    def random_portfolios(self, number_of_portfolios=1000):
        ...
        portfolios.drop(
            portfolios[(portfolios[self.tickers].sum(axis=1) > 1)].index, inplace=True
        )
        self.portfolios = portfolios

    def r(self, risk, epsilon=0.005):
        rng = self.portfolios[
            (self.portfolios["risk"] > risk - epsilon)
            & (self.portfolios["risk"] < risk + epsilon)
        ]
        return rng
    ...
    def display_plot(
        ...
        plot = self.portfolios.hvplot.scatter(
            x="risk",
            y="return",
            grid=True,
    ...
    def select_portfolio_by_risk(self, level=0, as_list=False):
        ...
        candidate = (
            self.portfolios[
                np.isclose(
                    self.portfolios["risk"], steps[level], rtol=steps[1] - steps[0]
                )
            ]
            .sort_values("return")
            .iloc[-1]
    ...
    def oracle(self, risk=None):
        ...
        separate = [
            self.asset_DataFrame[self.asset_DataFrame["symbol"] == symbol]
            for symbol in self.asset_DataFrame["symbol"].unique()
        ]
        data = pd.concat(
            separate, axis=1, join="inner", keys=self.asset_DataFrame["symbol"].unique()
        )
...
def run(tickers, budget=100000, risk=None):
    ...
    lady_in_red = tank.fetch(
        tickers=tickers, timeframe="1D", start="2017-5-9", end="2022-5-13"
    )
